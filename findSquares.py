...
def main():
    image = cv2.imread('foo-otsu.png', cv2.IMREAD_COLOR)
    if image is None :
        exit(1)
    ...
    #KO
    image_hight, image_width, image_colar = image.shape
    cv2.namedWindow('Square Detector', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Square Detector', 1000, int(1000*image_hight/image_width))
    #KO
    cv2.imshow('Square Detector', rimage)
    cv2.imwrite('foo-otsuSquares.png', rimage)
    c = cv2.waitKey()
    return 0;
